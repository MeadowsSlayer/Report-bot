        ...
        if statTime == "9:00" or statTime == "13:00" or statTime == "18:00":
            for beta in user:
                if u=="0":
                    ...
                    u="1"
                if dateTime==("Monday"):
                    cursor.execute("SELECT DateTime, Action, PlaceProject, Action2, Road FROM Table_2 ORDER BY ID LIMIT ?",(str(beta)))
...
def welcome_message(message):
    ...
    for beta in admins:
        bot.send_message(beta,"Вы являетесь админом и можете заносить новых пользователей с помощью команды /newperson.")
...
def other(message):
    ...
    action = message.text
    place = "0"
    cursor.execute("INSERT INTO Table_2 VALUES(%r,%i,%r,%r,%r,%r,%r,%r)" % (str(UserName_2), message.chat.id, str(UserID_1), time.asctime(), action, place, action2, null))
    conn.commit()
...
def mesage_reaction(message):
    ...
    if road == "1":
        ...
        cursor.execute("INSERT INTO Table_2 VALUES(%r,%i,%r,%r,%r,%r,%r,%r)" % (str(UserName_2), message.chat.id, str(UserID_1), time.asctime(), action, place, action2, road))
        conn.commit()
    if r == "0":
        pass
    elif r == "1":
